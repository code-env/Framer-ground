...
const existingFolders = fs.readdirSync("fg");
...
(async () => {
  ...
  if (!fs.existsSync(`fg/${variables.TypePath}`)) {
    fs.mkdirSync(`fg/${variables.TypePath}`);
    shouldRegister = true;
  }
  ...
  if (fs.existsSync(`fg/${variables.TypePath}/${variables.Filename}.tsx`)) {
    console.log(
      chalk.red(
        `File "fg/${variables.TypePath}/${variables.Filename}.tsx" already exists.`
      )
    );
    return;
  }
  ...
  createCopy(
    "component",
    `fg/${variables.TypePath}/${variables.Filename}.tsx`,
    variables
  );

  createCopy(
    "story",
    `fg/${variables.TypePath}/${variables.Filename}.stories.tsx`,
    variables
  );
